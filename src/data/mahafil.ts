...
export type MahfilStatus = "active" | "inactive" | "upcoming";
...
export interface MahfilCategory {
  id: string;
  name: string;
  description: string;
  icon: string;
...
export const mahfilCategories: MahfilCategory[] = [
  {
...
export const sampleMahafil: Mahfil[] = [
  {
    id: "mahfil-1",
...
export const getMahfilById = (id: string): Mahfil | undefined => {
  return sampleMahafil.find((m) => m.id === id);
};

export const getMahfilsByProvince = (provinceId: string): Mahfil[] => {
  return sampleMahafil.filter((m) => m.provinceId === provinceId);
};

export const getMahfilsByCity = (cityId: string): Mahfil[] => {
  return sampleMahafil.filter((m) => m.cityId === cityId);
};

export const searchMahafil = (query: string): Mahfil[] => {
  const lowerQuery = query.toLowerCase();
  return sampleMahafil.filter(
    (m) =>
      m.name.includes(query) ||
      m.description.includes(query) ||
      m.teacherName.includes(query) ||
      m.features.some((f) => f.includes(query))
  );
};
